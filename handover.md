...
- カードの高さ
  - 現状：画面枠の80~90%
  - 修正：自分で数値を変えて試したい
- 横方向（左右）にスクロールできてしまう。ちょっとズレる感じがある→固定したい
- 色
  - 文字の色：白、グレー？
  - カードの色：暗め？
  - 背景の色：明るめ？
...
## パターンBの試行
- パターン A：【画面分割型（PC・縦書き・横スクロール）】
  デスクトップでは、画面全体を一つの大きな「書見台」と見立て、その中で原稿を横に流します。
  構造: 本文全体（.manuscript-entry）を一つの長い Flex コンテナとします。
  CSS: height: 80vh; と flex-direction: row; を使い、文章が右から左へ流れる「巻物」を作り、横スクロールさせます。
  メリット: 縦書きの小説を読む体験として最も優れており、「行の高さが短い」という問題は発生しません（高さは画面いっぱいに固定されるため）。
...
- スマホ向けレイアウトは一旦リセット。`docs/assets/css/style.scss` の 640px 以下ブロックを最小構成（縦スクロール縦書き1カラム）に整理し、`docs/assets/js/manuscript-mobile.js` も削除。段落は `<p>` のまま行間を広げて読みやすさを優先。
